import {
  ...
  WebGLRenderTarget
} from "./chunk-AO72BDPG.js";

// node_modules/realism-effects/dist/index.js
...
var keepMaterialMapUpdated = (mrtMaterial, originalMaterial, prop, define, useKey) => {
  if (useKey) {
...
var updateVelocityDepthNormalMaterialBeforeRender = (c, camera) => {
  var _c$skeleton;
...
var updateVelocityDepthNormalMaterialAfterRender = (c, camera) => {
  var _c$skeleton2;
...
var createGlobalDisableIblRadianceUniform = () => {
  ...
  const globalIblRadianceDisabledUniform2 = {
    value: false
  };
  ShaderLib.physical.uniforms.iblRadianceDisabled = globalIblRadianceDisabledUniform2;
  const {
    clone
  } = UniformsUtils;
  UniformsUtils.clone = (uniforms) => {
    ...
    if ("iblRadianceDisabled" in uniforms) {
      result.iblRadianceDisabled = globalIblRadianceDisabledUniform2;
    }
    return result;
  };
  return globalIblRadianceDisabledUniform2;
};
var createGlobalDisableIblIradianceUniform = () => {
  ...
  const globalIblIrradianceDisabledUniform2 = {
    value: false
  };
  ShaderLib.physical.uniforms.iblIrradianceDisabled = globalIblIrradianceDisabledUniform2;
  const {
    clone
  } = UniformsUtils;
  UniformsUtils.clone = (uniforms) => {
    ...
    if ("iblIrradianceDisabled" in uniforms) {
      result.iblIrradianceDisabled = globalIblIrradianceDisabledUniform2;
    }
    return result;
  };
  return globalIblIrradianceDisabledUniform2;
};
...
var isChildMaterialRenderable = (c, material = c.material) => {
...
var materialProps = ["vertexTangent", "vertexColors", "vertexAlphas", "vertexUvs", "uvsVertexOnly", "supportsVertexTextures", "instancing", "instancingColor", "side", "flatShading", "skinning", "doubleSided", "flipSided"];
var copyNecessaryProps = (originalMaterial, newMaterial) => {
  for (const props of materialProps)
    newMaterial[props] = originalMaterial[props];
};
...
var TemporalReprojectMaterial = class extends ShaderMaterial {
  constructor(textureCount = 1, temporalReprojectCustomComposeShader = "") {
    let finalFragmentShader2 = fragmentShader$5.replace("#include <reproject>", reproject);
    if (typeof temporalReprojectCustomComposeShader === "string") {
      finalFragmentShader2 = finalFragmentShader2.replace("temporalReprojectCustomComposeShader", temporalReprojectCustomComposeShader);
    }
    let definitions = "";
    for (let i = 0; i < textureCount; i++) {
      definitions += /* glsl */
    ...
    finalFragmentShader2 = definitions + finalFragmentShader2.replaceAll("textureCount", textureCount);
    finalFragmentShader2 = unrollLoops(finalFragmentShader2);
    const matches = finalFragmentShader2.matchAll(/inputTexture\[\s*[0-9]+\s*]/g);
    for (const [key] of matches) {
      const number = key.replace(/[^0-9]/g, "");
      finalFragmentShader2 = finalFragmentShader2.replace(key, "inputTexture" + number);
    }
    const matches2 = finalFragmentShader2.matchAll(/accumulatedTexture\[\s*[0-9]+\s*]/g);
    for (const [key] of matches2) {
      const number = key.replace(/[^0-9]/g, "");
      finalFragmentShader2 = finalFragmentShader2.replace(key, "accumulatedTexture" + number);
    }
    const matches3 = finalFragmentShader2.matchAll(/gOutput\[\s*[0-9]+\s*]/g);
    for (const [key] of matches3) {
      const number = key.replace(/[^0-9]/g, "");
      finalFragmentShader2 = finalFragmentShader2.replace(key, "gOutput" + number);
    }
    super({
      ...
      vertexShader,
      fragmentShader: finalFragmentShader2,
      blending: NoBlending,
      depthWrite: false,
      depthTest: false,
      toneMapped: false,
...
var g = 1.324717957244746;
var a1 = 1 / g;
var a2 = 1 / (g * g);
var base = 1.1127756842787055;
var generateR2 = (count) => {
  const points = [];
...
var defaultTemporalReprojectPassOptions = {
  blend: 0.9,
  dilation: false,
  constantBlend: false,
...
var tmpProjectionMatrix$1 = new Matrix4();
var tmpProjectionMatrixInverse$1 = new Matrix4();
var TemporalReprojectPass = class extends Pass {
  constructor(scene, camera, velocityDepthNormalPass, textureCount = 1, options = defaultTemporalReprojectPassOptions) {
    ...
    if (options.neighborhoodClamp)
      this.fullscreenMaterial.defines.neighborhoodClamp = "";
    if (options.logTransform)
      this.fullscreenMaterial.defines.logTransform = "";
    ...
    for (const opt of ["reprojectSpecular", "neighborhoodClamp"]) {
      ...
      if (typeof value !== "array")
        value = Array(textureCount).fill(value);
      this.fullscreenMaterial.defines[opt] = /* glsl */
      `bool[](${value.join(", ")})`;
    }
  ...
  render(renderer) {
    ...
    if (this._camera.view)
      this._camera.view.enabled = false;
    ...
    if (this._camera.view)
      this._camera.view.enabled = true;
  ...
  jitter(jitterScale = 1) {
    this.unjitter();
    if (this.r2Sequence.length === 0)
      this.r2Sequence = generateR2(256).map(([a, b]) => [a - 0.5, b - 0.5]);
  ...
  unjitter() {
    if (this._camera.clearViewOffset)
...
var defaultTRAAOptions = {
  blend: 0.8,
  constantBlend: true,
  dilation: true,
  logTransform: false,
  // ! todo: check if can use logTransform without artifacts
  neighborhoodClampRadius: 2,
  depthDistance: 10,
  worldDistance: 5,
  neighborhoodClamp: true
};
var TRAAEffect = class extends Effect {
  constructor(scene, camera, velocityDepthNormalPass, options = defaultTRAAOptions) {
    super("TRAAEffect", traa_compose, {
      ...
      uniforms: /* @__PURE__ */ new Map([["inputTexture", new Uniform(null)]])
    });
    this._scene = scene;
    this._camera = camera;
    options = {
  ...
  update(renderer, inputBuffer) {
    ...
    for (const mesh of noJitterMeshes) {
      ...
      if (!(renderData != null && renderData.programs))
        continue;
...
var useEdgeStoppingTypes = [["moment", "", "useMoment"], ["depth", "depthPhi", "useDepth"], ["normal", "normalPhi", "useNormal"], ["roughness", "roughnessPhi", "useRoughness"]];
var DenoisePass = class extends Pass {
  constructor(camera, textures = [], options = defaultDenoisePassOptions) {
    ...
    this.fullscreenMaterial.defines.roughnessDependent = /* glsl */
    ...
    this.fullscreenMaterial.defines.basicVariance = /* glsl */
...
`;
    ...
    let finalFragmentShader2 = definitions + fragmentShader$4.replace("#include <denoiseCustomComposeShaderFunctions>", this.options.denoiseCustomComposeShaderFunctions).replace("#include <denoiseCustomComposeShader>", this.options.denoiseCustomComposeShader).replace("#include <outputShader>", outputShader).replaceAll("textureCount", this.textures.length).replaceAll("momentTextureCount", Math.min(this.textures.length, 2));
    finalFragmentShader2 = unrollLoops(finalFragmentShader2);
    const matches = finalFragmentShader2.matchAll(/inputTexture\[\s*[0-9]+\s*]/g);
    for (const [key] of matches) {
      const number = key.replace(/[^0-9]/g, "");
    ...
    this.fullscreenMaterial.fragmentShader = finalFragmentShader2;
  ...
  keepEdgeStoppingDefinesUpdated() {
    for (const [name, phi, define] of useEdgeStoppingTypes) {
      var _this$fullscreenMater;
      const useEdgeStoppingType = this.options[name] && (phi === "" || ((_this$fullscreenMater = this.fullscreenMaterial.uniforms[phi]) == null ? void 0 : _this$fullscreenMater.value) > 1e-3);
...
var defaultSVGFTemporalReprojectPassOptions = {
  fullAccumulate: true,
  temporalReprojectCustomComposeShader: svgf_temporal_reproject
};
var SVGFTemporalReprojectPass = class extends TemporalReprojectPass {
...
var CubeToEquirectEnvPass = class extends Pass {
  constructor() {
...
var workerOnMessage = ({
  ...
  function colorToLuminance(r, g2, b) {
  ...
  const gatherData = (data2, width2, height2, flipY2, marginalDataArray2, conditionalDataArray2) => {
    if (flipY2) {
      for (let y = 0, h = height2 - 1; y <= h; y++) {
        for (let x = 0, w = width2 * 4; x < w; x += 4) {
          const newY = h - y;
          const ogIndex = y * w + x;
          const newIndex = newY * w + x;
          data2[newIndex] = data2[ogIndex];
          data2[newIndex + 1] = data2[ogIndex + 1];
          data2[newIndex + 2] = data2[ogIndex + 2];
          data2[newIndex + 3] = data2[ogIndex + 3];
        }
      }
    }
    const pdfConditional = new Float32Array(width2 * height2);
    const cdfConditional = new Float32Array(width2 * height2);
    const pdfMarginal = new Float32Array(height2);
    const cdfMarginal = new Float32Array(height2);
    let totalSumValue2 = 0;
    ...
    for (let y = 0; y < height2; y++) {
      ...
      for (let x = 0; x < width2; x++) {
        const i = y * width2 + x;
        const r = data2[4 * i + 0];
        const g2 = data2[4 * i + 1];
        const b = data2[4 * i + 2];
        const weight = colorToLuminance(r, g2, b);
        cumulativeRowWeight += weight;
        totalSumValue2 += weight;
        pdfConditional[i] = weight;
        cdfConditional[i] = cumulativeRowWeight;
      }
      if (cumulativeRowWeight !== 0) {
        for (let i = y * width2, l = y * width2 + width2; i < l; i++) {
    ...
    for (let i = 0; i < height2; i++) {
      const dist = (i + 1) / height2;
      const row = binarySearchFindClosestIndexOf(cdfMarginal, dist);
      marginalDataArray2[i] = (row + 0.5) / height2;
    }
    for (let y = 0; y < height2; y++) {
      for (let x = 0; x < width2; x++) {
        const i = y * width2 + x;
        const dist = (x + 1) / width2;
        const col = binarySearchFindClosestIndexOf(cdfConditional, dist, y * width2, width2);
        conditionalDataArray2[i] = (col + 0.5) / width2;
      }
    }
    return totalSumValue2;
  };
  if (!isFloatType) {
...
var blob = new Blob(["onmessage = " + workerOnMessage], {
...
var SSGIMaterial = class extends ShaderMaterial {
  constructor() {
    super({
      type: "SSGIMaterial",
      uniforms: {
      ...
      fragmentShader: fragmentShader$3.replace("#include <ssgi_utils>", ssgi_utils).replace("#include <sampleBlueNoise>", sampleBlueNoise),
      vertexShader,
      blending: NoBlending,
      depthWrite: false,
      depthTest: false,
...
var backgroundColor$2 = new Color(0);
var overrideMaterial = new MeshDepthMaterial({
  depthPacking: RGBADepthPacking,
  side: BackSide
});
var BackSideDepthPass = class extends Pass {
...
var backgroundColor$1 = new Color(0);
var SSGIPass = class extends Pass {
  constructor(ssgiEffect, options) {
    ...
    if (ssgiEffect._camera.isPerspectiveCamera)
  ...
  setMRTMaterialInScene() {
    ...
    for (const c of this.visibleMeshes) {
      ...
      mrtMaterial.uniforms.roughness.value = this.ssgiEffect.selection.size === 0 || this.ssgiEffect.selection.has(c) ? origRoughness : 1e11;
  ...
  render(renderer) {
    ...
    if (this.ssgiEffect.autoThickness)
...
var globalIblIrradianceDisabledUniform = createGlobalDisableIblIradianceUniform();
var globalIblRadianceDisabledUniform = createGlobalDisableIblRadianceUniform();
var SSGIEffect = class extends Effect {
  /**
  ...
  constructor(scene, camera, velocityDepthNormalPass, options = defaultSSGIOptions) {
    ...
    super("SSGIEffect", ssgi_compose, {
      ...
      uniforms: /* @__PURE__ */ new Map([["inputTexture", new Uniform(null)], ["sceneTexture", new Uniform(null)], ["depthTexture", new Uniform(null)], ["toneMapping", new Uniform(NoToneMapping)]])
    });
    ...
    if (options.diffuseOnly) {
      ...
      options.basicVariance = [25e-5, 25e-5];
    }
    ...
    RenderPass.prototype.render = function(...args) {
      if (this !== ssgiRenderPass) {
        ...
        if (wasUsingRenderPass != th.isUsingRenderPass)
          th.updateUsingRenderPass();
      }
  ...
  update(renderer, inputBuffer) {
    ...
    this.usingRenderPassRAF = requestAnimationFrame(() => {
      ...
      if (wasUsingRenderPass != this.isUsingRenderPass)
        this.updateUsingRenderPass();
    });
  }
};
SSGIEffect.DefaultOptions = defaultSSGIOptions;
var SSREffect = class extends SSGIEffect {
  constructor(scene, camera, velocityDepthNormalPass, options = defaultSSGIOptions) {
    options = {
      ...defaultSSGIOptions,
      ...options
...
var SSDGIEffect = class extends SSGIEffect {
  constructor(scene, camera, velocityDepthNormalPass, options = defaultSSGIOptions) {
    options = {
      ...defaultSSGIOptions,
      ...options
...
var MotionBlurEffect = class extends Effect {
  constructor(velocityPass, options = defaultOptions) {
    ...
    super("MotionBlurEffect", motion_blur, {
      ...
      uniforms: /* @__PURE__ */ new Map([["inputTexture", new Uniform(null)], ["velocityTexture", new Uniform(velocityPass.texture)], ["blueNoiseTexture", new Uniform(null)], ["blueNoiseSize", new Uniform(new Vector2())], ["texSize", new Uniform(new Vector2())], ["intensity", new Uniform(1)], ["jitter", new Uniform(1)], ["frame", new Uniform(0)], ["deltaTime", new Uniform(0)]]),
      defines: /* @__PURE__ */ new Map([["samples", options.samples.toFixed(0)], ["samplesFloat", options.samples.toFixed(0) + ".0"]])
    });
    this.pointsIndex = 0;
    this.makeOptionsReactive(options);
  }
  ...
  update(renderer, inputBuffer, deltaTime) {
    ...
    this.uniforms.get("deltaTime").value = Math.max(1 / 1e3, deltaTime);
...
var VelocityDepthNormalMaterial = class extends ShaderMaterial {
  constructor() {
    super({
...
var tmpProjectionMatrixInverse = new Matrix4();
var VelocityDepthNormalPass = class extends Pass {
...
var VelocityPass = class extends VelocityDepthNormalPass {
  constructor(scene, camera) {
...
function generateDenoiseSamples(numSamples, numRings, r, texelSize) {
  ...
  for (let i = 0; i < numSamples; i++) {
    const v = new Vector2(Math.cos(angle), Math.sin(angle)).multiplyScalar(Math.pow(radius, 0.75)).multiply(texelSize).multiplyScalar(r);
    samples.push(v);
    radius += radiusStep;
    angle += angleStep;
  }
...
var finalFragmentShader$1 = fragmentShader$2.replace("#include <sampleBlueNoise>", sampleBlueNoise);
var defaultPoissonBlurOptions = {
  iterations: 1,
  radius: 8,
  rings: 5.625,
  lumaPhi: 10,
...
var PoissionDenoisePass = class extends Pass {
...
var AOPass = class extends Pass {
  constructor(camera, scene, fragmentShader2) {
    super();
    this._camera = camera;
    this._scene = scene;
    this.renderTarget = new WebGLRenderTarget(1, 1, {
    ...
    const finalFragmentShader2 = fragmentShader2.replace("#include <sampleBlueNoise>", sampleBlueNoise);
    this.fullscreenMaterial = new ShaderMaterial({
      fragmentShader: finalFragmentShader2,
      vertexShader,
      uniforms: {
...
var finalFragmentShader = fragmentShader$1.replace("#include <hbao_utils>", hbao_utils);
var HBAOPass = class extends AOPass {
...
var AOEffect = class extends Effect {
  constructor(composer, camera, scene, aoPass, options = defaultAOOptions) {
    super("AOEffect", ao_compose, {
      type: "FinalAOMaterial",
      uniforms: /* @__PURE__ */ new Map([["inputTexture", new Uniform(null)], ["depthTexture", new Uniform(null)], ["power", new Uniform(0)], ["color", new Uniform(new Color("black"))]])
    });
    this.lastSize = {
      width: 0,
      height: 0,
      ...
      Object.defineProperty(this, key, {
        ...
        set(value) {
          ...
          switch (key) {
            ...
            case "depthPhi":
            case "normalPhi":
              this.poissionDenoisePass.fullscreenMaterial.uniforms[key].value = Math.max(value, 1e-4);
              break;
            default:
...
var HBAOEffect = class _HBAOEffect extends AOEffect {
...
var SSAOPass = class extends AOPass {
  constructor(camera, scene) {
...
var SSAOEffect = class _SSAOEffect extends AOEffect {
  constructor(composer, camera, scene, options = {}) {
    _SSAOEffect.DefaultOptions = {
      ...AOEffect.DefaultOptions,
      ...{
        spp: 16,
        distance: 1,
...
//# sourceMappingURL=realism-effects.js.map
